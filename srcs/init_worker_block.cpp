...
template<typename T>
void    setDirectives(T &directives,Worker &worker,int *found_index)
{
	for (size_t i = 0; i < directives.size(); i++)
	{
...
void   init_worker_block(Worker &worker, std::string &host ,std::string &path,std::vector<ServerBlocks> &serverBlocks, int &is_dir, int &is_regular,int fd,std::map<int, int> &matched_server_block)
{
    ...
    setDirectives(worker.getBlockWorker().getDirectives(),worker,&found_index);
    setDirectives(worker.getLocationWorker().getDirectives(),worker,&found_index);
